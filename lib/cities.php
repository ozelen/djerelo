...
class Cities extends HotCat{
	...
	function Exec($name, $params=NULL){
		...
		switch($name){
			...
			case "weather" : return $this->showWeather( $identcity ); break;
			/*
			case "infrastructure" : 
	...
	function getByCond($cond, $wide_info=false){
		...
		$q = "
			select 
				...
				lat, lng
				,	concat('c-', bookit.Id) as BookitId
			...
			left join Regions dis on st.`district_id` = dis.Id
			left join PageData dd on dd.PageId = dis.PageId and dd.Lang = '$lang'
			left join bookit_CitiesSearchForm bookit on bookit.djSettlementId = st.Id
			where 1=1
			$conditions
			group by st.Id
		";
	...
	function getByQuery($q, $wide_info=false){
		...
		while($city = mysql_fetch_object($cts)){
			...
			//$weather = $this->showWeather( $city->Ident );
			//dbg($weather);
			...
			$xml.='
				<city id="'.$city->Id.'" lat="'.$city->lat.'" lng="'.$city->lng.'">
					...
					'.$near.'
					'.$comments.'
					<bookit id="'.$city->BookitId.'"><![CDATA['.$city->BookitName.']]></bookit>
				</city>
			';
		}
		if(!$xml) return "";
	...
	function showWeather($id){
		...
		if(file_exists($file_path)){
			...
			return $xml;
		}//else dbg("no weather at $file_path");
	}
