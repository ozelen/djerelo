...
class Objects extends HotCat{
	...
	function Exec($name, $params=NULL){
		...
		switch($name){
			case "show": 
				//dbg("test");
				return $this->Show($this->uriVars['objid']); 
			break;
			...
			case "toplist" : return $this->topList($p_arr); break;
		}
	}
	...
	function Show($id){
		...
		if($this->leftTime($id)>=0 || $role=='admin' || $this->CFG->HDB->getFieldWhere("bookit_hotelsBookit", "id", "where djObjId = '$id'"))
	...
	function leftTime($id){
		...
		$now = time();
		return $this->LeftDays = $exp_time-$now;
	}
	...
	function cache__Show($id){
		...
		$q="
			...
			,	concat('o-', bookit.Id) as BookitId
			,	bookit.Name as BookitName

		--	, TIMESTAMPDIFF(DAY, now(), Expire) as RRate -- if agreements join
			...
			--	left join Rating r on r.ObjId = obj.Id and r.Owner = 'Objects' and r.Name = 'SocialsTotal'
				...
				left join bookit_hotelsBookit bookit on bookit.djObjId = obj.Id
		...
		while($o = mysql_fetch_array($obj)){
			...
			$xml.='
				<object id="'.$o['Id'].'" page="'.$o['PageId'].'" viewmode="'.$ViewModeAttr.'" format="full">
					...
					<rate>'.$this->getRate($o['Id']).'</rate>
					<topic id="'.$o['Topic'].'"/>
					<bookit id="'.$o['BookitId'].'"><![CDATA['.$o['BookitName'].']]></bookit>
					<leave>'.$this->LeftDays.'</leave>
					'.$page_xml.'
					'.$location.'
					'.$catmenu_xml.'
					'.$modules_xml.'
	...
	function miniInfo($p, $format='xml'){
		$db = $this->CFG->HDB;
		//dbg('mini');
		if(is_array($p))
		...
		else if(is_numeric($p))$cond_str=" and obj.Id = $p ";
		...
		while($o = mysql_fetch_array($objs)){
			$xml.='
				<object id="'.$o['ObjId'].'" page="'.$o['PageId'].'" format="cut">
					...
					<registered>'.$o['DateRegistered'].'</registered>
					<name><![CDATA['.$o['Name'].']]></name>
					<contacts><![CDATA['.autoLinks($o['Contacts']).']]></contacts>
					<info><![CDATA['.autoLinks($o['Info']).']]></info>
					<image>
						<name><![CDATA['.$o['ImgName'].']]></name>
						<ext>'.$o['ImgExt'].'</ext>
					</image>
					...
					<rate>'.$this->getRate($o['ObjId']).'</rate>
					<bookit id="'.$o['BookitId'].'"><![CDATA['.$o['BookitName'].']]></bookit>
					<leave>'.$this->LeftTime($o['Id']).'</leave>
				</object>
			';
			$obj_arr[]=array(
					'id'	=> $o['ObjId'],
					'name' 	=> $o['Name'],
					'city'	=> array('id'=>$o['Settlement'], 	'name'=>$o['CityName']),
					'type'	=> array('id'=>$o['TypeId'], 		'name'=>$o['TypeName']),
					'prof'	=> array('id'=>$o['ProfId'], 		'name'=>$o['ProfName']),
				);
		}
		//dbg($xml);
		$xml = "<objects>".$xml."</objects>";
		return $format=='array' ? $obj_arr : $xml;
	}

	function topList($p){
		//dbg($p);
		$p['lang']=$this->CFG->lang;
		foreach($p as $k=>$v){$key.="$k=$v";}
		$crypt = md5($key); // dbg("$key = $crypt");
		$cache = new Cache(array('dir'=>'toplists/', 'fname'=>"$crypt.xml", 'ForAllUsers'=>true));
		$xml = $cache->get();
		$xml = ($get = $cache->get()) ? $get : $cache->put($this->fastList($p));
		return $xml;
		//dbg($xml);
	}
	...
	function fastList($p){
		...
		foreach($p as $key => $value){
			switch($key){
				...
				case 'limit'	 : $limit = " limit ".$value; break;
				case 'name'      : $listname = "<name>$value</name>"; break;

			}
		}
		//dbg($order_cond);
		$city_cond = $cityid ? "and obj.Settlement = $cityid" : '';
		$order_cond = $order_cond ? $order_cond : "order by Expire desc";
		...
		$q="
			select 
				...
				r.Val as SocRate
				, TIMESTAMPDIFF(DAY, now(), Expire) as RRate
				, round((r.Val*15 + TIMESTAMPDIFF(DAY, now(), Expire))/2) as avgRate
			...
			from 
				...
				left join Pages cont_page on cont_page.Rozdil = obj.PageId and cont_page.Name = 'contacts'
				left join PageData cont_data on cont_data.PageId = cont_page.Id and cont_data.Lang = '$lang'
		...
		while($obj = mysql_fetch_object($objs)){
			...
			$xml.='
				<obj id="'.$obj->Id.'">
					...
					<contacts><![CDATA['.$obj->Contacts.']]></contacts>
					...
					<city id="'.$obj->setid.'" ident="'.$obj->setident.'">'.$obj->setname.'</city>
					...
					<currency>uah</currency>
					<rate>'.$obj->SocRate.'</rate>
					<rating>
						<rank id="social">'.$obj->SocRate.'</rank>
						<rank id="rrate">'.$obj->RRate.'</rank>
						<rank id="average">'.$obj->avgRate.'</rank>
					</rating>
					<bookit id="'.$o['BookitId'].'"><![CDATA['.$o['BookitName'].']]></bookit>
					'.$price.'
	...
	function getRate($id){
		if(!$id)return;
		$last_record = mysql_fetch_object($this->CFG->HDB->q("select * from Rating where Owner = 'Objects' and ObjId = $id and Name = 'SocialsTotal' order by Updated desc limit 1"));
		return $last_record->Val;
	}
}
