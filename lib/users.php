...
class UserData extends MNG{
	...
	function getData($ln='', $pw=''){
		...
		$q = "select * from Users where Email = '$ln' and Pass = '$pw' ";
		...
		//dbg("GET [$ln,$pw] UID: $this->Id DATA: $this->Email:$this->Pass");
		$this->getXML();
	}
	function getXML(){
		return $this->XML = '
	...
	function Login($ln, $pw, $direct=''){
		...
		//if($_POST['keystring'] == $_SESSION['captcha_keystring']){
			if($this->UserData->Id){
				...
				setcookie("djerelo.uid", 	$this->UserData->Id);
				setcookie("djerelo.uname", 	$this->UserData->Name);
				setcookie("djerelo.pass", 	$this->UserData->Pass);
				setcookie("djerelo.email", 	$this->UserData->Email);
						
				header("Location: http://$direct/");
				$this->CFG->SysMsg->Add('Welcome, '.$this->UserData->Name.'!', "ok");
			}else{
				$this->CFG->SysMsg->Add("Incorrect Login/Password", "error");
			}
